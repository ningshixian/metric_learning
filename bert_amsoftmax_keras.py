...
def bert_model_amsoftmax():
    ...
    first_token = Dropout(dropout_rate, name="dp1")(first_token)
    ...
    train_model.compile(optimizer=Adam(learning_rate),)  # 用足够小的学习率
    train_model.summary()
...
with session_global.as_default():
    with session_global.graph.as_default():  # 用with新建一个graph，这样在运行完以及异常退出时就会释放内存
        ...
        # model.load_weights("it_nlu-best-online.h5")  # 直接测试模型
